...
%         end
        for cc=1:3
            imC=im1(:,:,cc);
            medianC(cc)=median(double(imC(I))); % 确保medianC(cc)是双精度
            imMarked(:,:,cc)=double(mask)*medianC(cc) ...
                +(1-double(mask)).*double(imMarked(:,:,cc)); % 转换为双精度进行计算
        end
        hold off;
        imshow(imMarked);
        x=[];y=[];
    ...
    elseif (button==99);% 'c'
        ...
        medianC(1)=colorRGB(cy,cx,1)*255;
        medianC(2)=colorRGB(cy,cx,2)*255;
        medianC(3)=colorRGB(cy,cx,3)*255;
         mask=0;
        for i=2:length(x)
            x0=x(i-1);y0=y(i-1);
            x1=x(i);y1=y(i);
